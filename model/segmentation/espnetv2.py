...
class ESPNetv2Segmentation(nn.Module):
    ...
    def __init__(
        ...
        self.bu_dec_l2 = EfficientPyrPool(
            in_planes=dec_planes[0],
            proj_planes=pyr_plane_proj,
            out_planes=dec_planes[1],
        )
        self.bu_dec_l3 = EfficientPyrPool(
            in_planes=dec_planes[1],
            proj_planes=pyr_plane_proj,
        ...
        self.bu_br_l3 = nn.Sequential(
            nn.BatchNorm2d(dec_planes[1]), nn.PReLU(dec_planes[1])
        )
        self.bu_br_l4 = nn.Sequential(
            nn.BatchNorm2d(dec_planes[2]), nn.PReLU(dec_planes[2])
        )
    ...
    def get_segment_params(self):
        modules_seg = [
            ...
            self.bu_dec_l3,
            self.bu_dec_l4,
            self.merge_enc_dec_l4,
            self.merge_enc_dec_l3,
            self.merge_enc_dec_l2,
            self.bu_br_l4,
            self.bu_br_l3,
            self.bu_br_l2,
        ]
...
def espnetv2_seg(args):
    ...
    if weights:
        ...
        if os.path.isfile(weights):
            ...
            print_error_message(
                "Weight file does not exist at {}. Please check. Exiting!!".format(
                    weights
                )
            )
            exit()
        ...
        if len(overlap_dict) == 0:
            ...
            exit()
        print_info_message(
            "{:.2f} % of weights copied from basenet to segnet".format(
                len(overlap_dict) * 1.0 / len(model_dict) * 100
            )
        )
...
if __name__ == "__main__":
    ...
    args.s = 2.0
    args.weights = (
        "../classification/model_zoo/espnet/espnetv2_s_2.0_imagenet_224x224.pth"
    )
    args.dataset = "pascal"
